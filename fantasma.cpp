   ...
   // cout << "Lista final ordenada...: X: " << k->getX() << " Y: " << k->getY() << endl;
	...
	while(k->getPaiY() != this->posicaoY || k->getPaiX() != this->posicaoX)
	{
		...
		i=0;
		 for ( u = vertices.begin(); u != vertices.end(); u++ )
		{
			if(u->getX() == k->getPaiX() && u->getY() == k->getPaiY())
			{
				k = u;
				break;
			}
	...
	else if((*mapa).getPosicao(this->posicaoX, this->posicaoY) == COMIDAGF)
	{
		(*mapa).setComidaGrande(this->posicaoX, this->posicaoY);
	}
	else
	{
	...
	else if((*mapa).getPosicao(u->getX(), u->getY()) == COMIDAG)
	{
		(*mapa).setFantasmaComidaG(u->getX(), u->getY());
	}
	else
	{
